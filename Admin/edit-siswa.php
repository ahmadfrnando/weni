...
$nisn = $_GET['nisn'];
include '../koneksi.php';
$sql = "SELECT*FROM siswa,spp,kelas WHERE siswa.id_kelas=kelas.id_kelas AND siswa.id_spp=spp.id_spp AND 
nisn='$nisn'";
...
<a href="?url=siswa" class="btn btn-primary btn-sm">Kembali</a>
<hr>
<form method="post" action="?url=proses-edit-siswa&nisn=<?= $nisn; ?>">
    ...
    <div class="form-group mb-2">
        <label>Alamat</label>
        <textarea name="alamat" class="form-control"><?= $data['alamat'] ?><?= $data['alamat'] ?></textarea>
    </div>
    <div class="form-group mb-2">
        <label>SPP</label>
