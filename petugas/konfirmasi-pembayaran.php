<?php
session_start();
if (empty($_SESSION['id_petugas']) || $_SESSION['level'] != 'petugas') {
    echo "<script>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$data) {
        echo "<script>alert('Data pembayaran tidak ditemukan.'); window.location.href='petugas.php';</script>";
        exit();
    }
    ...
    if (mysqli_query($koneksi, $query_pembayaran) && mysqli_query($koneksi, $query_update)) {
        echo "<script>alert('Pembayaran telah dikonfirmasi!'); window.location.href='petugas.php';</script>";
    } else {
        echo "<script>alert('Gagal mengkonfirmasi pembayaran.'); window.location.href='petugas.php';</script>";
    }
}
